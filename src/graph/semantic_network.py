...
class SemanticNetwork:
    ...
    def update(self, em_proportion=0.1, g_proportion=0.1, include_set=None, stop_set=None, thresh=0.7, verbose=False):
        if not include_set and not stop_set:
            em_subset = np.array(
                [idx for idx in _select_subset(
                    self.embedding_matrix, self.embedding_matrix.shape[0], subset_proportion=em_proportion
                ) if self.index_to_key[idx]]
            )
            g_subset = np.array(
                [idx for idx in _select_subset(
        ...
        elif include_set and not stop_set:
            em_subset = np.array(
                [idx for idx in _select_subset(
                    self.embedding_matrix, self.embedding_matrix.shape[0], subset_proportion=em_proportion
                ) if self.index_to_key[idx] in include_set]
            )
            g_subset = np.array(
                [idx for idx in _select_subset(
                    self.graph.adjacency_matrix, len(self.graph.nodes), subset_proportion=g_proportion
        ...
        elif not include_set and stop_set:
            em_subset = np.array(
                [idx for idx in _select_subset(
                    self.embedding_matrix, self.embedding_matrix.shape[0], subset_proportion=em_proportion
            ...
            em_subset = np.array(
                [idx for idx in _select_subset(
                    self.embedding_matrix, self.embedding_matrix.shape[0], subset_proportion=em_proportion
                ) if self.index_to_key[idx] in include_set and self.index_to_key[idx] not in stop_set]
            )
            g_subset = np.array(
                [idx for idx in _select_subset(
                    self.graph.adjacency_matrix, len(self.graph.nodes), subset_proportion=g_proportion
